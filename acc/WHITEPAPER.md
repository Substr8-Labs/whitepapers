...
Before presenting the detailed architecture, we establish ACC's formal foundations.

### Capability Definition

A **capability** in ACC is a cryptographically-signed authorization token defined as:

```
Capability := (resource, action, constraints, expiry, issuer, signature)
```

Where:
- **resource**: The target object or service (e.g., `file:/home/user/docs/*`, `api:weather.example.com`)
- **action**: The permitted operation (e.g., `read`, `write`, `execute`, `invoke`)
- **constraints**: Additional restrictions (e.g., `rate_limit:100/hour`, `time_window:9am-5pm`)
- **expiry**: Timestamp after which the capability becomes invalid
- **issuer**: The entity that granted the capability (cryptographic identity)
- **signature**: Ed25519 signature over all fields, binding the capability to its issuer

### Fundamental Properties

**No Ambient Authority.** In ACC, agents possess *no implicit permissions*. Every action requires presenting an explicit capability. There is no "default access"—an agent without capabilities can perform no operations. This eliminates confused deputy attacks where authority is accidentally inherited from the execution context.

*Formally:* Let `A` be an agent and `R` a resource. `A` may access `R` if and only if `A` possesses a valid capability `C` where `C.resource ⊇ R` and `C.action` includes the requested operation.

**Monotonic Attenuation.** Capabilities can only be *weakened*, never strengthened, when delegated. If Agent A delegates to Agent B, the delegated capability must be a subset of A's capability.

*Formally:* For any delegation `C → C'`:
- `C'.resource ⊆ C.resource`
- `C'.action ⊆ C.action`  
- `C'.constraints ⊇ C.constraints` (more restrictive)
- `C'.expiry ≤ C.expiry`

This property ensures that privilege cannot escalate through delegation chains.
...
> **Citation Note:** [Putta et al., 2024] refers to "Agent Q: Advanced Reasoning and Learning for Autonomous AI Agents" — the specific claims about agent authentication patterns require verification against the primary source.
...
- **[Dennis & Van Horn, 1966]** Dennis, J. B., & Van Horn, E. C. "Programming Semantics for Multiprogrammed Computations." Communications of the ACM, 1966.
- **[Putta et al., 2024]** Agent Q: Advanced Reasoning and Learning for Autonomous AI Agents. arXiv.org 2024. *[Citation requires verification — source discusses agent reasoning, not access control patterns]*
...
- **[Mihajlović & Njeguš, 2025]** Multi-agent Ai For Adaptive Treasury And Capital Optimization. Proceedings of the 12th International Scientific Conference - FINIZ 2025.
